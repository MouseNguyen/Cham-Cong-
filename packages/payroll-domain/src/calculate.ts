...
export function calculatePayroll(input: PayrollInput): PayrollResult {
  ...
  const holiday = calculateHoliday(input.employment.monthlySalaryVnd, input.holiday);
  const grossVnd = input.employment.monthlySalaryVnd + holiday.entitlementVnd + holiday.premiumVnd;
  const pit = calculatePit(grossVnd, employeeInsuranceVnd, input.pit);
  const lines: BaseLine[] = [line('ordinary.monthly_salary', 'Lương tháng', input.employment.monthlySalaryVnd, input, 'FULL_TIME_MONTHLY_SALARY')];
  ...
  const formulaTrace=lines.filter(l=>l.code.startsWith('ordinary.')||l.code.startsWith('insurance.')).map(l=>{
    ...
    return {code:l.code,amountVnd:l.amountVnd,operands:fund&&policy?{baseVnd:fund.appliedBaseVnd.toString(),requestedBaseVnd:policy.baseVnd.toString(),minimumVnd:policy.minimumVnd.toString(),maximumVnd:policy.maximumVnd.toString(),eligible:policy.eligible,rateBasisPoints:l.code.startsWith('insurance.employee.')?policy.employeeRateBasisPoints:policy.employerRateBasisPoints,denominator:'10000',exactNumerator:(fund.appliedBaseVnd*BigInt(l.code.startsWith('insurance.employee.')?policy.employeeRateBasisPoints:policy.employerRateBasisPoints)).toString(),componentDate:policy.componentDate}:{monthlySalaryVnd:input.employment.monthlySalaryVnd.toString(),formula:'monthly_salary'}};
  });
  ...
  const auditedLines: CalculationLine[]=lines.map(l=>{
    ...
    let formula='monthlySalaryVnd';
    if(l.code.startsWith('insurance.'))formula='appliedBaseVnd * rateBasisPoints / 10000';
    if(l.code==='holiday.entitlement')formula='0 (included in monthly salary)';
    if(l.code==='holiday.daytime_premium')formula='monthlySalaryVnd * payableMilliseconds * daytimePremiumBasisPoints / (ordinaryHourlyDivisorHours * 3600000 * 10000)';
...
function validateInput(input: PayrollInput): void {
  ...
  if(input.policy.holidayEntitlementTreatment!=='included_in_monthly'||!Number.isSafeInteger(input.policy.holidayPremiumBasisPoints)||input.policy.holidayPremiumBasisPoints<30000||input.policy.holidayPremiumBasisPoints>100000)throw new Error('HOLIDAY_POLICY_INVALID');
  ...
  if (input.employment.kind !== 'full_time') throw new Error('EMPLOYMENT_KIND_UNSUPPORTED');
...
function syntheticPolicyContent(input:PayrollInput):unknown {
  return {kind:'synthetic_materialized_policy',id:input.ruleBinding.id,version:input.ruleBinding.version,policy:input.policy,
    funds:input.insuranceFunds.map(f=>({id:f.id,minimumVnd:f.minimumVnd,maximumVnd:f.maximumVnd,employeeRateBasisPoints:f.employeeRateBasisPoints,employerRateBasisPoints:f.employerRateBasisPoints})).sort((a,b)=>a.id<b.id?-1:a.id>b.id?1:0),
...
const JSON_KEYS:Record<string,readonly string[]>={
  FIXTURE_INVALID:['mode','calculator','employment','ruleBinding','attendance','insuranceFunds','pit','holiday','policy','unsupportedComponents'],
  ...
  FIXTURE_INSURANCE_INVALID:['id','eligible','baseVnd','minimumVnd','maximumVnd','employeeRateBasisPoints','employerRateBasisPoints','componentDate'],
