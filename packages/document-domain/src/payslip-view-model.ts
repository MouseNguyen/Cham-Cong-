...
export function payslipViewModel(source:PayslipSource,payslipId:string,issuedAt:Date):PayslipViewModel {
  ...
  const currentDuration=snapshot.totalPayableDurationMs,legacyDuration=snapshot.approvedPayableMilliseconds;
  if(currentDuration!==undefined&&legacyDuration!==undefined&&currentDuration!==legacyDuration)throw Error('DOCUMENT_SOURCE_BINDING_MISMATCH');
  const snapshotDuration=currentDuration!==undefined?currentDuration:legacyDuration;
  if(compensation.schema!==undefined&&compensation.schema!=='compensation-v1')throw Error('DOCUMENT_SCOPE_UNSUPPORTED');
  const compensationTerm=compensation.schema==='compensation-v1'?object(compensation.compensation):compensation;
  if(compensation.schema==='compensation-v1'&&compensationTerm.basis!=='monthly_salary')throw Error('DOCUMENT_SCOPE_UNSUPPORTED');
  if(compensationTerm!==compensation&&compensation.monthlySalaryVnd!==undefined&&compensation.monthlySalaryVnd!==compensationTerm.monthlySalaryVnd)throw Error('DOCUMENT_SOURCE_BINDING_MISMATCH');
  const bindings: [unknown,unknown][] = [[result.inputHash,source.input_hash],[result.rulePackHash,source.rule_pack_hash],[result.rulePackId,source.rule_pack_id],[rule.id,source.rule_pack_id],[result.calculatorVersion,source.calculator_version],[calculator.version,source.calculator_version],[result.calculatorArtifactHash,source.calculator_artifact_hash],[calculator.sourceArtifactSha256,source.calculator_artifact_hash],[result.canonicalizationVersion,source.canonicalization_version],[calculator.canonicalizationVersion,source.canonicalization_version],[result.resultSchemaVersion,source.result_schema_version],[employment.monthlySalaryVnd,compensationTerm.monthlySalaryVnd],[attendance.payableMilliseconds,snapshotDuration]];
