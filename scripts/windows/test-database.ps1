...
function run(exe,args,input,allowFail=false,childEnv=env){
 const r=cp.spawnSync(exe,args,{cwd:root,env:childEnv,encoding:'utf8',input,timeout:args.some(a=>a.includes('integration/admin/')&&a.endsWith('browser-runner.cjs'))?900000:args.some(a=>a.endsWith('browser-runner.cjs'))?240000:120000,windowsHide:true,maxBuffer:8*1024*1024,stdio:path.basename(exe)==='pg_ctl.exe'?'ignore':'pipe'});
...
(async()=>{
 try{
  ...
  try{
   ...
   if(mode==='Green'||taskId==='PAY-W5-02b'||taskId==='PAY-W5-02a'||taskId==='PAY-W6-01'||taskId==='PAY-W6-02b'){
