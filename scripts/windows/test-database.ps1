param([ValidateSet('Red','Green')][string]$Mode='Green', [ValidateSet('PAY-W2-01','PAY-W6-02a','PAY-W2-02','PAY-W2-03','PAY-W3-01b','PAY-W3-02a','PAY-W3-02b','PAY-W7-02a','PAY-W3-03','PAY-W4-01','PAY-W5-02a','PAY-W6-01','PAY-W6-02b')][string]$TaskId='PAY-W2-01', [switch]$Browser)
...
(async()=>{
 try{
  ...
  try{
   ...
   if(mode==='Green'||taskId==='PAY-W5-02a'||taskId==='PAY-W6-01'||taskId==='PAY-W6-02b'){
  ...
  const tests=run(process.execPath,['node_modules/vitest/vitest.mjs','run',taskId==='PAY-W6-02b'?'tests/integration/delivery/recipient-isolation.spec.ts':taskId==='PAY-W6-01'?'tests/integration/documents':taskId==='PAY-W4-01'?'tests/integration/pay-runs':taskId==='PAY-W3-03'?'tests/integration/attendance/review.spec.ts':kioskTask?(taskId==='PAY-W7-02a'?'tests/integration/operations/service-lifecycle.windows.spec.ts':'tests/integration/attendance/ingress.spec.ts'):taskId==='PAY-W3-01b'?'tests/integration/attendance/schedule-effective-dates.spec.ts':taskId==='PAY-W2-03'?'tests/integration/employees':taskId==='PAY-W2-02'?'tests/integration/auth':'tests/integration/db',...(taskId==='PAY-W6-02a'?['tests/integration/delivery/outbox.spec.ts','tests/integration/delivery/leases.spec.ts','tests/integration/delivery/retry.spec.ts']:[]),...(taskId==='PAY-W6-02b'&&mode==='Green'?['tests/integration/delivery/gmail-contract.spec.ts']:[]),'--maxWorkers=1','--no-file-parallelism'],undefined,true);
  receipt.tests={exit:tests.exit,passed_count:Number(tests.output.match(/Tests\s+(\d+) passed/)?.[1]||0)};
  if(mode==='Red'){
   const expectedRed=taskId==='PAY-W6-02b'?tests.output.includes('DELIVERY_RELEASE_NOT_IMPLEMENTED'):taskId==='PAY-W6-01'?tests.output.includes('DOCUMENT_NOT_IMPLEMENTED'):taskId==='PAY-W4-01'
