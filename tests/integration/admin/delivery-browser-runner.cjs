...
if (target.protocol !== 'postgresql:' || target.hostname !== '127.0.0.1' || target.port !== '55432' || target.pathname !== '/payslip_w5_02b_synthetic' || target.username !== 'payslip_app') throw Error('OWNED_ADMIN_DATABASE_REQUIRED');
...
Object.assign(env, {
 ...
 PAYSLIP_ADMIN_ORIGIN: origin, PAYSLIP_ADMIN_TEST_ORIGIN: origin, PAYSLIP_DB_TASK_ID: 'PAY-W5-02b',
...
env.PAYSLIP_DOCUMENT_STORAGE=path.join(root,'.tmp/PAY-W5-02b/documents',crypto.randomUUID());
env.PAYSLIP_QPDF_PATH=path.join(root,'.tools/qpdf/12.4.1/qpdf-12.4.1-mingw64/bin/qpdf.exe');
env.PAYSLIP_CHROMIUM_PATH=path.join(root,'.tools/playwright/chromium-1243/chrome-win64/chrome.exe');
env.PAYSLIP_DELIVERY_SEED_FILE=path.join(root,'.tmp/PAY-W5-02b',crypto.randomUUID()+'.json');
...
(async()=>{
 try {
  ...
  const seed=cp.spawnSync(process.execPath,[path.join(root,'node_modules/vitest/vitest.mjs'),'run','tests/integration/admin/delivery-http.spec.ts','--maxWorkers=1','--no-file-parallelism'],{cwd:root,env:{...env,PAYSLIP_DELIVERY_SEED:'1'},windowsHide:true,encoding:'utf8',timeout:120000});
  if(seed.status!==0){console.error(clean(seed.stdout));console.error(clean(seed.stderr));throw Error('DELIVERY_SEED_FAILED');}
  env.PAYSLIP_ORGANIZATION_ID=JSON.parse(fs.readFileSync(env.PAYSLIP_DELIVERY_SEED_FILE,'utf8')).organizationId;
  await freePort();
  ...
  const result=await runTests(red?['tests/e2e/delivery.spec.ts','--grep','finalized delivery panel','--reporter=json']:['tests/e2e/delivery.spec.ts','--reporter=line']);
  if(red){
   ...
   if(report.errors?.length||actual.length!==1||failure?.status!=='failed'||!failure.errors?.some(e=>e.message?.includes('Giao phiếu lương')&&e.message?.includes('toBeVisible')))throw Error('ADMIN_BEHAVIOR_RED_NOT_PROVEN');
   fs.writeFileSync(path.join(root,'ops/evidence/PAY-W5-02b-red.json'),JSON.stringify({layer:'behavior_specific_red',status:'passed',expected:'finalized pay-run delivery heading',actual:'missing delivery panel on finalized pay run',test:specs[0].title,all_current_migrations_applied:true},null,2)+'\n');
   console.log('ADMIN_BEHAVIOR_RED_OBSERVED');
  }else {
   if(result.code!==0)throw Error('ADMIN_BROWSER_TEST_FAILED');
   const http=cp.spawnSync(process.execPath,[path.join(root,'node_modules/vitest/vitest.mjs'),'run','tests/integration/admin/delivery-http.spec.ts','--maxWorkers=1','--no-file-parallelism'],{cwd:root,env,windowsHide:true,encoding:'utf8',timeout:120000,maxBuffer:2*1024*1024});
