...
export async function seedM0Destination(pool:Pool,organizationId:string,employeeId:string){
 const destinationId=randomUUID(),address=`m0-${randomUUID()}@example.invalid`,payload=JSON.stringify({address,channel:'email'});
 await withTransaction(pool,async tx=>{
  await tx.query("INSERT INTO delivery_destinations(id,organization_id,employee_id,channel,address,valid_from,valid_to,canonical_payload,content_hash) VALUES($1,$2,$3,'email',$4,'2026-01-01','2027-01-01',$5,$6)",[destinationId,organizationId,employeeId,address,payload,createHash('sha256').update(payload).digest('hex')]);
  const job=await enqueueSyntheticVerification(tx,{organizationId,employeeId,destinationId,idempotencyKey:randomUUID(),now:new Date()});
  await tx.query("INSERT INTO employee_email_verifications(id,organization_id,employee_id,destination_id,job_id,code_hash,expires_at,verified_at) VALUES($1,$2,$3,$4,$5,$6,'2027-01-01',now())",[randomUUID(),organizationId,employeeId,destinationId,job.jobId,'0'.repeat(64)]);
...
export function adminPool(): Pool {
  ...
  if (
    ...
    || url.port !== '55432'
    || url.pathname !== '/payslip_m0_synthetic'
...
export async function seedAttendanceEvidence(pool: Pool, fixture: AdminFixture, employeeId: string) {
  ...
  await pool.query("INSERT INTO public_holidays(id,organization_id,holiday_date,name,source_ref) VALUES($1,$2,'2026-08-04','M0 synthetic holiday','synthetic:M0:PD05:not-a-statutory-calendar')",[randomUUID(),fixture.organizationId]);
  for (const [direction, occurredAt] of [['IN', '2026-08-04T01:00:00.000Z'], ['OUT', '2026-08-04T03:00:00.000Z']] as const) {
