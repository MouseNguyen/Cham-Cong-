...
if (target.protocol !== 'postgresql:' || target.hostname !== '127.0.0.1' || target.port !== '55432' || target.pathname !== '/payslip_m0_synthetic' || target.username !== 'payslip_app') throw Error('OWNED_ADMIN_DATABASE_REQUIRED');
...
const inputTemplate = JSON.parse(fs.readFileSync(path.join(root,'tests/fixtures/payroll/full-time-holiday-synthetic.json'),'utf8')).input;
...
// Configuration supplies policy, never employee attendance. Snapshot must supply two holiday hours.
inputTemplate.holiday.payableMilliseconds='0';
Object.assign(env, {
 ...
 PAYSLIP_ADMIN_ORIGIN: origin, PAYSLIP_ADMIN_TEST_ORIGIN: origin, PAYSLIP_DB_TASK_ID: 'PAY-M0',
...
env.PAYSLIP_DOCUMENT_STORAGE=path.join(root,'.tmp/PAY-M0/documents',crypto.randomUUID());
...
(async()=>{
 try {
  ...
  const result=await runTests(red?['tests/e2e/admin-flows.spec.ts','--grep','authenticated dashboard exposes payroll work queue','--reporter=json']:['tests/e2e/m0.spec.ts','--reporter=line']);
  if(red){
   ...
   fs.writeFileSync(path.join(root,'ops/evidence/PAY-M0-red.json'),JSON.stringify({layer:'behavior_specific_red',status:'passed',expected:'authenticated dashboard work-queue heading',actual:'missing heading on baseline dashboard',test:specs[0].title,all_current_migrations_applied:true},null,2)+'\n');
   console.log('ADMIN_BEHAVIOR_RED_OBSERVED');
  }else {
   fs.writeFileSync(path.join(root,'ops/evidence/PAY-M0-browser-result.json'),JSON.stringify({exit:result.code,output:clean(result.stdout),error:clean(result.stderr)},null,2)+'\n');
   if(result.code!==0)throw Error('M0_BROWSER_TEST_FAILED');
