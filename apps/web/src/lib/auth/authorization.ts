...
export type Command = "PAYROLL_DRAFT"|"ATTENDANCE_DRAFT"|"ATTENDANCE_APPROVE"|"PAY_RUN_APPROVE"|"PAY_RUN_FINALIZE"|"PAYSLIP_RELEASE"|"CLOCK_EVENT"|"EMPLOYEE_READ"|"EMPLOYEE_MANAGE"|"EMPLOYEE_DEACTIVATE";
export const FRESH_MFA_MS=10*60*1000;
const commands=new Set<string>(["PAYROLL_DRAFT","ATTENDANCE_DRAFT","ATTENDANCE_APPROVE","PAY_RUN_APPROVE","PAY_RUN_FINALIZE","PAYSLIP_RELEASE","CLOCK_EVENT","EMPLOYEE_READ","EMPLOYEE_MANAGE","EMPLOYEE_DEACTIVATE"]);
...
export function authorize(actor:Actor,command:Command,now=new Date(),resource?:{organizationId:string}):void{
 ...
 if(command==="PAYROLL_DRAFT"||command==="ATTENDANCE_DRAFT"||command==="EMPLOYEE_READ"||command==="EMPLOYEE_MANAGE")return;
